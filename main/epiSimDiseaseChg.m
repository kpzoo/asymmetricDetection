...
function [Iday, Lam, Rtrue, tday, Iwarn, Pomega] = epiSimDiseaseChg(scenNo, epiNo, tday, nday, remGap, chgpt)
...
scenNam = {'constant', 'cyclic', 'logistic', 'switch', 'boom-bust', 'bottle',...
    '2-step', 'filtered', 'flip'};
...
switch(scenNo)
    ...
    case 9
        % Same as switch but start then settle to R=1
        tch = chgpt; Rtrue = zeros(1, nday);
        Rtrue(1:round(tch(1)/2)) = 2;
        Rtrue(round(tch(1)/2)+1:tch(1)) = 1;
        Rtrue(tch(1)+1:tch(2)) = 2.5; 
        Rtrue(tch(2)+1:end) = 0.5;
end
