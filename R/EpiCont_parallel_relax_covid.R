...
episim_data_ens[[1]] <- Epi_MPC_run_wd_K(episim_data_ens[[1]], Epi_pars, Noise_pars, Action_space0, pred_days = pred_days, start_day = 1, n_ens = n_ens, ndays = 62L, R_est_wind = R_est_wind, pathogen = 1, susceptibles = 0, delay = 1, ur = 1, r_dir = 1, N = N)
...
results <- pblapply(1:sim_ens, function(jj) {
  episim_data_ens[[jj]] <- Epi_MPC_run_wd_K(episim_data_ens[[jj]], Epi_pars, Noise_pars, Action_space, pred_days = pred_days, start_day = 61L, n_ens = n_ens, ndays = nrow(episim_data), R_est_wind = R_est_wind, pathogen = 1, susceptibles = 0, delay = 1, ur = 1, r_dir = 1, N = N)
}, cl=cl)
...
#save(episim_data_ens, file = "episim_data_ens_relax_with_noise_1000.RData")
